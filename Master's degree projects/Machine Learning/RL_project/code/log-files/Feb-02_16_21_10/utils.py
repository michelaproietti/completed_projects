...
class Scaler(object):
    ...
    def update(self, x): # x: NumPy array, shape = (N, obs_dim)
        # for more information see: https://stats.stackexchange.com/questions/43159/how-to-calculate-pooled-variance-of-two-groups-given-known-group-variances-mean
        if self.first_pass:
...
class Logger(object): # Simple training logger: saves to file and optionally prints to stdout
    ...
    def disp(log):
        ...
        for key in log_keys:
            if key[0] != '_':  # don't display log items that starts with '_'
