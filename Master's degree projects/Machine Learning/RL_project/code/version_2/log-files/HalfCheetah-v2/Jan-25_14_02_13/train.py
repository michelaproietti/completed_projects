...
def run_policy(env, policy, scaler, logger, episodes):
    ...
    if logger is not None : logger.log({'_MeanReward': np.mean([t['rewards'].sum() for t in trajectories]),
                'Steps': total_steps})
...
def main(env_name, num_episodes, gamma, lam, kl_targ, batch_size, hid1_mult, init_logvar):
    ...
    while episode < num_episodes:
        ...
        name = f"logs/TRPO-HalfCheetah-v2-{current_time}"
        val_func.fit(observes, disc_sum_rew, logger, episode, name)  # update value function
