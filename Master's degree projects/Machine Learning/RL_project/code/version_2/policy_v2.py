...
class Policy(object):
    def __init__(self, obs_dim, act_dim, kl_targ, hid1_size, init_logvar):
        ...
        self.trpo = TRPO(obs_dim, act_dim, hid1_size, kl_targ, init_logvar)  # initialize TRPO agent
    ...
    def update(self, observes, actions, advantages, logger):
        ...
        # In this last part many numbers have been obtained empirically.
        # The beta is dynamically adjusted to achieve the KL goal and the learning rate is adjusted according to the beta value
...
# Neural network for policy approximation function. The policy is parametrized by Gaussian means and variances.
# Trainable variables hold log-variances for each action dimension, which means that variances are not determined by the NN.
class PolicyNN(Layer):
    ...
    def __init__(self, obs_dim, act_dim, hid1_size, init_logvar, **kwargs):
        ...
        def model_sequential():
            model = Sequential()
            model.add(Dense(hid1_units, activation=tf.nn.leaky_relu, input_shape=(obs_dim,)))
            model.add(Dense(hid2_units, activation=tf.nn.leaky_relu, input_shape=(hid1_units,)))
            model.add(Dense(hid3_units, activation=tf.nn.leaky_relu, input_shape=(hid2_units,)))
